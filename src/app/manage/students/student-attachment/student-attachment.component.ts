...
export class StudentAttachmentComponent implements OnInit {
  ...
  imgPreview(file: PersonAttachment) {
    ...
    if (file.fileType == 1) {
      img[
        'src'
      ] = `${this.apiUrl}/filebyid/${file.attachmentId}?dt=attach&type=3`;
    } else if (
      file.fileType == 2 ||
      file.fileType == 11 ||
      file.fileType == 15
      ...
      open(
        `${this.apiUrl}/filebyid/${file.attachmentId}?dt=attach&type=0`,
        '_self'
      );
    }
  }
  ...
  deleteFile(file: PersonAttachment): void {
    this.attachmentService.deletePersonAttachment(file.id).subscribe((iRet) => {
      if (iRet > 0) {
        let message =
          'The file "' +
          this.selected.name +
        ...
        this.messageService.add({
          severity: 'info',
          summary: 'Info',
          detail: 'No file has been deleted.',
          life: 10000,
        });
      }
    });
