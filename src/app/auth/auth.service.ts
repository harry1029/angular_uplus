...
export class AuthService {
  ...
  login(userLogin: UserLogin): Observable<UserLogin> {
    ...
    return this.http.post<UserLogin>(url, userLogin, this.httpOptions).pipe(
      ...
      catchError(
        this.handleHttpErrorService.handleError<UserLogin>(
          `AuthService: login =${userLogin.userName}`
        )
      )
    );
  }
  ...
  getUserClaims(userId: number): Observable<any> {
    return this.http
      .get<any>(`${this.apiUrl}/hs/${userId}?dt=userclaims`)
      .pipe(
        catchError(
          this.handleHttpErrorService.handleError<UserLogin>(
            `AuthService: updateUserClaims =${userId}`
          )
        )
      );
  }
